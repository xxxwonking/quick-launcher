...
- 最近使用显示已加入设置项，默认关闭；关闭时搜索框空输入保持空列表，开启后只展示本次成功使用过的应用。
- 自定义命令支持新增、编辑、启用/禁用、删除，启用后立即合并到搜索目录并支持固定网址或网页搜索。
- macOS 应用索引适配：扫描 `/Applications`、`~/Applications`、`/System/Applications`，并合并 `mdfind` 返回的 `.app` Bundle。

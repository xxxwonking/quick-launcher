...
export function GuidedTour({ open, steps, onComplete, onSkip, onClose }: GuidedTourProps): React.JSX.Element | null {
  ...
  return (
    <div aria-label="使用教程" aria-modal="true" className="fixed inset-0 z-[100]" role="dialog">
      {!spotlight ? <div className="absolute inset-0 bg-black/35" data-testid="tour-full-mask" /> : null}
      {spotlight ? (
        <div
          className="pointer-events-none fixed rounded-2xl ring-2 ring-accent shadow-[0_0_0_9999px_rgba(0,0,0,0.28),0_0_24px_var(--accent-glow)] transition-all"
          data-testid="tour-spotlight"
          style={spotlight}
        />
      ) : null}
      <section
        className="fixed w-[360px] rounded-2xl border border-window bg-popover p-5 text-primary shadow-2xl"
        style={{ top: cardTop, left: cardLeft }}
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="mb-1 text-xs font-semibold text-accent">第 {state.stepIndex + 1} 步 / 共 {state.stepCount} 步</p>
            <h2 className="text-lg font-bold">{step.title}</h2>
          </div>
          <button aria-label="关闭教程" className="rounded-lg p-1.5 text-secondary hover:bg-hover hover:text-primary" onClick={() => { dispatch({ type: 'close' }); onClose?.() }} type="button">
        ...
        <div className="mt-5 flex items-center justify-between border-t border-divider pt-4">
          <button className="text-xs font-medium text-secondary hover:text-primary" onClick={() => { dispatch({ type: 'skip' }); onSkip?.() }} type="button">
            跳过教程
          </button>
          <div className="flex gap-2">
            <button className="tour-button" disabled={state.stepIndex === 0} onClick={() => dispatch({ type: 'previous' })} type="button">
              <ArrowLeft aria-hidden="true" className="size-4" />上一步
            </button>
            <button className="tour-button tour-button-primary" onClick={next} type="button">
              {isFinal ? '完成' : '下一步'}<ArrowRight aria-hidden="true" className="size-4" />
            </button>
          </div>
        </div>
      </section>
